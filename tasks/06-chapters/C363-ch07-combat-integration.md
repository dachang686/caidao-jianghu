...
完成第7章至少 2 类普通敌人、章节 Boss“榜司督主”、系统解锁“结局路线锁定”和章节级集成测试。
...
- 榜司督主 具有明确阶段、可读意图和最多一个专属反套路规则；失败可原地重试。
- 本章引入或自然使用至少 1 个规则幽默模块，并配置 1 个 Boss 专属开场或败北 PresentationCue。
- Boss 胜利后原子交付奖励、解锁“结局路线锁定”、写自动档并开放下一章/结局。
...
- 已完成第7章2类普通敌人、榜司督主双阶段 Boss、规则/情境/互动/演出四层覆盖、奖励结算、系统解锁和章节 E2E。
...
- `pnpm simulate:battles -- --ch07-ranking-governor --start=1 --end=100` 通过：95胜、5负、0超时、最大12回合、issues为空、阶段最多转换1次。
