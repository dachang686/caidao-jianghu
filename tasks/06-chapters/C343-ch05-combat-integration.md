---
id: C343
title: 第5章 西域驿路：敌人、驿路双煞 与章节集成
phase: chapter-content
depends_on: [C342, G105, G107, G115, G119, H223, H225, S241, S242, S243, S245]
status: done
...
完成第5章至少 2 类普通敌人、章节 Boss“驿路双煞”、系统解锁“自建门派与 Tick 派遣”和章节级集成测试。
...
- 驿路双煞 具有明确阶段、可读意图和最多一个专属反套路规则；失败可原地重试。
- 本章引入或自然使用至少 1 个规则幽默模块，并配置 1 个 Boss 专属开场或败北 PresentationCue。
- Boss 胜利后原子交付奖励、解锁“自建门派与 Tick 派遣”、写自动档并开放下一章/结局。
...
- 已完成第5章2类普通敌人、驿路双煞双阶段 Boss、规则/情境/互动/演出四层覆盖、奖励结算、系统解锁和章节 E2E。
- 已接入存档、下一章解锁、Boss 重试、战斗 UI 与章节模拟器。

## 验证记录

- 2026-08-12：驿路任务、对白、热点、两类普通敌人与双煞 Boss 均由章节运行时驱动；`e2e/ch05-08-flow.spec.ts` 桌面/手机关键路径通过。
- `pnpm lint` 通过。
...
- `pnpm simulate:battles -- --ch05-twin-bandits --start=1 --end=100` 通过：99胜、1负、0超时、最大11回合、issues为空、阶段最多转换1次。
