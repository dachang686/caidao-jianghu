...
import { assertGameSaveV2Fixture, makeGameSaveV2Fixture, SNAPSHOT_CHAPTERS } from './fixtures/game-save-v2'
...
test.describe('GameSaveV2 chapter snapshots', () => {
  for (const chapterId of SNAPSHOT_CHAPTERS) {
    test(`${chapterId} fixture passes schema and exposes chapter metadata`, async ({ page }) => {
      ...
      expect(save.runtime.screen).toBe('jianghu')
      expect(save.runtime.world.currentChapter).toBe(chapterId)
