  ...
  return (
    <main className={`battle-screen battle-screen--complete${phaseTwo ? ' is-phase-two' : ''}`}>
      <header className="battle-top battle-top--complete">
        <div><span className="battle-breadcrumb">{isBangsi ? '清河县 · 百晓榜告示台' : isBlackwindLeader ? '黑风寨 · 山寨门' : isQingyunMaster ? '青云山 · 山门石阶' : isTwinBandits ? '西域驿路 · 驼队车场' : isTideMaster ? '东海镇 · 潮声寺' : isRankingGovernor ? '京城 · 百晓榜司' : isRankingMaster ? '武林大会 · 评判台' : '小愚村 · 悦来客栈擂台'}</span><h1>先看意图，再出刀</h1></div>
      ...
      <div className="battle-content">
        <section className="battle-arena" aria-label="战斗场景">
          ...
          <BattleCombatantCard combatant={{ name: battle.enemy.name, side: 'enemy', sprite: enemySprite, hp: battle.enemy.hp, maxHp: battle.enemy.maxHp, qi: battle.enemy.qi, maxQi: battle.enemy.maxQi, posture: battle.enemyPosture, statuses: toStatusViews(battle.enemy.statuses), subtitle: phaseTwo ? (isBangsi ? '反盖一印，仍然会诚实明示。' : isBlackwindLeader ? '反卷山河，仍然会诚实明示。' : isQingyunMaster ? '剑谱纠错，仍然会诚实明示。' : isLateBoss ? '终局动作仍然会诚实明示。' : '三成认真，仍然会诚实出招。') : (isBangsi ? '公文正派，意图暂时不藏。' : isBlackwindLeader ? '山寨冲榜，意图暂时不藏。' : isQingyunMaster ? '门面工程，意图暂时不藏。' : isLateBoss ? '公开验收，意图暂时不藏。' : '名门正派，意图暂时不藏。'), phase: battle.enemy.phase }} />
        </section>
        <BattleLog logs={battle.logs.slice(-50)} />
        <BattleSkillBar skills={skillViews} />
      </div>

      {battle.turn === 'victory' && <section className="battle-outcome battle-outcome--victory" data-testid="battle-victory" role="dialog" aria-labelledby="battle-victory-title"><span className="battle-outcome__eyebrow">结算完成</span><h2 id="battle-victory-title">险胜，但挺像那么回事</h2><p>{isBangsi ? '获得经验 58、银两 72 和清河榜牌；装备、采集、锻造已解锁。' : isBlackwindLeader ? '获得经验 72、银两 90 和黑风寨令；技能树、烹饪已解锁。' : isQingyunMaster ? '获得经验 86、银两 110 和青云名帖；意图进阶、装备强化已解锁。' : isTwinBandits ? '获得经验 96、银两 130 和西域封条；自建门派、Tick 派遣已解锁。' : isTideMaster ? '获得经验 108、银两 150 和潮声珠；委托进阶、门人事件已解锁。' : isRankingGovernor ? '获得经验 120、银两 180 和京城公牍；结局路线锁定已解锁。' : isRankingMaster ? '获得经验 150、银两 220 和大会印记；四结局、通关后继续已解锁。' : '获得经验 42、银两 50、生锈菜刀和称号「菜刀新秀」。'}</p><Button onClick={leaveBattle}>{isBangsi ? '回到清河县' : isBlackwindLeader ? '回到黑风寨' : isQingyunMaster ? '回到青云山' : isTwinBandits ? '回到西域驿站' : isTideMaster ? '回到东海镇' : isRankingGovernor ? '回到京城' : isRankingMaster ? '回到武林大会' : '回到小愚村'}</Button></section>}
      {battle.turn === 'victory' && <PresentationCue definition={presentationCue} actionId={`battle:victory:${lastLog?.id ?? battle.round}`} reducedMotion={settings.reducedMotion} muted={settings.masterMuted || !settings.sillySfxEnabled} label={isBangsi ? '榜下捕快盖章认输，清河榜牌开始记账。' : isBlackwindLeader ? '黑风寨主收起空旗，败北也被登记在榜。' : isQingyunMaster ? '青云掌门收起折扇，承认门面验收通过。' : isLateBoss ? `${battle.enemy.name}收起证据，承认验收通过。` : '白大侠抱拳，擂台木牌开始记账。'} />}
      {battle.turn === 'defeat' && <section className="battle-outcome battle-outcome--defeat" data-testid="battle-defeat" role="dialog" aria-labelledby="battle-defeat-title"><span className="battle-outcome__eyebrow">本局结束</span><h2 id="battle-defeat-title">江湖评价：普通韭菜 C-</h2><p>{isBangsi ? '榜下捕快把卷宗合上：失败不扣线索，原地重试即可。' : isBlackwindLeader ? '黑风寨主把空旗插回去：失败不扣账榜，原地重试即可。' : isQingyunMaster ? '青云掌门把剑谱合上：失败不扣名帖，原地重试即可。' : isLateBoss ? `${battle.enemy.name}把证据收好：失败不扣章节线索，原地重试即可。` : '白大侠看了看你的菜刀，说它至少很有勇气。资源会在重试时恢复。'}</p><div><Button className="menu-action--primary" data-testid="battle-retry" onClick={retryBattle}>原地重试</Button><Button onClick={leaveBattle}>{isBangsi ? '先回街市歇歇' : isBlackwindLeader ? '先回山寨歇歇' : isQingyunMaster ? '先回山门歇歇' : isLateBoss ? '先回章节歇歇' : '先回村歇歇'}</Button></div></section>}
    </main>
  )
}
