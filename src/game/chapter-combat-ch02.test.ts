...
describe('第 2 章榜下捕快胜利事务', () => {
  beforeEach(() => {
    useRootGameStore.getState().startNewGame('清河试刀客', 'reckless')
  })
  ...
  it('清河县快照可以完成调查、进入 Boss、失败原地重试并胜利返回场景', () => {
    ...
    useRootGameStore.setState((current) => ({
      ...current,
    ...
    expect(useRootGameStore.getState().battle?.turn).toBe('player')
    useRootGameStore.setState((current) => ({
      ...current,
      battle: current.battle ? { ...current.battle, enemy: { ...current.battle.enemy, hp: 1 } } : null,
    }))
    useRootGameStore.getState().useSkill('basicSlash')
    const settled = useRootGameStore.getState()
    ...
    expect(useRootGameStore.getState().world.currentChapter).toBe('ch02')
  })
})
